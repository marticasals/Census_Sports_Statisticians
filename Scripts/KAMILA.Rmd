...
```{r}
library(dplyr)
df.vars.all.complete.w5.km2 %>%
  group_by(Cluster) %>%
  summarise(median = median(`Total citations in GS`, na.rm = TRUE))
```
...
createTable(comp_tot_citations, show.p.mul=TRUE,show.descr = TRUE)

comp_tot_citations <- compareGroups(Cluster ~ `Total citations in GS`,  data = df.vars.all.complete.w5.km2, method = c(`Total citations in GS` = 2))
createTable(comp_tot_citations, show.p.mul = TRUE)
```

*Comparison with total citations since 2021*

```{r}
comp_tot_citations_since2021 <- compareGroups(Cluster ~ `Total citations in GS since 2021` , data = df.vars.all.complete.w5.km2)
createTable(comp_tot_citations, show.p.mul=TRUE)

comp_tot_citationssince2021 <- compareGroups(Cluster ~ `Total citations in GS since 2021`,  data = df.vars.all.complete.w5.km2, method = c(`Total citations in GS since 2021` = 2))
createTable(comp_tot_citationssince2021, show.p.mul = TRUE)
```
...
comp_tot_citations_acording_Hindex <- compareGroups(Cluster ~ `Total citations in GS acording H-index`,  data = df.vars.all.complete.w5.km2, method = c(`Total citations in GS acording H-index` = 2))
createTable(comp_tot_citations_acording_Hindex, show.p.mul = TRUE)
```

*Comparison with total citations according to the H-index since 2021*

```{r}
comp_tot_citations_acording_Hindex_since2021 <- compareGroups(Cluster ~ `Total citations in GS acording H-index since 2021` , data = df.vars.all.complete.w5.km2)
createTable(comp_tot_citations_acording_Hindex_since2021, show.p.mul=TRUE)

comp_tot_citations_acording_Hindex_since2021 <- compareGroups(Cluster ~ `Total citations in GS acording H-index since 2021`,  data = df.vars.all.complete.w5.km2, method = c(`Total citations in GS acording H-index since 2021` = 2))
createTable(comp_tot_citations_acording_Hindex_since2021, show.p.mul = TRUE)
```
...
comp_article_most_citations <- compareGroups(Cluster ~ `Number of citations of the article with most citations`,  data = df.vars.all.complete.w5.km2, method = c(`Number of citations of the article with most citations` = 2))
createTable(comp_article_most_citations, show.p.mul = TRUE)
```
...
(var_num1 <- ggplot(bd, aes(y = `Total citations in GS`, fill = Cluster)) + 
    geom_boxplot())
(var_num2 <- ggplot(bd, aes(y = `Total citations in GS since 2021`, fill = Cluster)) + 
    geom_boxplot())
(var_num3 <- ggplot(bd, aes(y = `Total citations in GS acording H-index`, fill = Cluster)) + 
    geom_boxplot())
(var_num4 <- ggplot(bd, aes(y = `Total citations in GS acording H-index since 2021`, fill = Cluster)) + 
    geom_boxplot())
...
```{r}
#library(openxlsx)
clusters <- df.vars.all.complete.w5.km2[c(3,32,40,47,64,70,38,7),c(1:5,15)]
clusters

#clusters <- df.vars.all.complete.w5.km2$Cluster
#write.xlsx(clusters, "mi_archivo.xlsx")
```
